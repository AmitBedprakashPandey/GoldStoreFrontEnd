...
import { updateInvoice,fetchAllInvoices } from "../Store/Slice/InvoiceWithoutGstSlice";
...
const GstBillwithoutPrint = ({ close}) => {
    const dispatch = useDispatch();
    const componentRef = useRef();
    const [paramdata, setParamData] = useState();    

    const { Invoices, loading } = useSelector((state) => state.InvoicesWithoutGst);
    ...
    useLayoutEffect(()=>{
        ...
        if (storedData) {
          try {
            ...
            dispatch(fetchAllInvoices(paramdata?.invoice)).then((doc) => console.log(doc)
            )
          } catch (error) {
            ...
            <table >
              ...
              <thead>
                <tr className="text-xs">
                  ...
                  <th className="uppercase text-center w-72 -pt-2 print:pt-3 pb-3">
                    Description Goods
                  </th>
...
export default GstBillwithoutPrint;