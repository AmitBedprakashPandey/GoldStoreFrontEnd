...
export const createInvoicesId = createAsyncThunk(
  "InvoiceWithoutGstId/create",
  async (BranchData, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${url}/invoicewithoutgstid`, BranchData, {
        headers: {
...
export const fetchOneInvoicesId = createAsyncThunk(
  "InvoiceWithoutGstId/fetchAll",
  async (id, { rejectWithValue }) => {
    try {
      const response = await axios.get(`${url}/invoicewithoutgstid/${localStorage.getItem('user')}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `${localStorage.getItem("token")}`,
        },
...
export const updateInvoicesId = createAsyncThunk(
  "InvoiceWithoutGstId/update",
  async (data, { rejectWithValue }) => {

    try {
      const response = await axios.put(
        `${url}/invoicewithoutgstid/${data}/${localStorage.getItem('user')}`,       
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `${localStorage.getItem("token")}`,
...
const InvoiceWithoutGstIdSlice = createSlice({
  name: "InvoiceId",
  initialState,
  reducers: {},
...
export default InvoiceWithoutGstIdSlice.reducer;
